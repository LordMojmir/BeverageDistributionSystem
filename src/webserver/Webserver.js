...
var Constructor = function Constructor() {
	...
	var publishOrdersList = function publishOrdersLists(list) {
		bus.publish(webapp.shared.topics.ACTUALORDER, list);
	};
	...
	var sendOrderListToClients = function sendOrderListToClients() {
		database.getAllDocumentsInCollection(ORDERS_COLLECTION).then(publishOrdersList);
	};
	...
	var addOrderToOrdersList = function addOrderToOrdersList(order){
		...
		database.insert(ORDERS_COLLECTION, order).then(sendOrderListToClients);
	...
	var  removeOrderFromOrdersList = function  removeOrderFromOrdersList(order){

		var removeOrder = function removeOrder(currentTableNumber) {
			database.remove(ORDERS_COLLECTION, order).then(sendOrderListToClients);
			return currentTableNumber !== order.table;
			
		};

 			console.log('This order was removed from the DB:', order, removeOrder(order));
		
			database.getAllDocumentsInCollection(COLLECTION_NAME).then(removeOrder);
	...
	var onDatabaseConnected = function onDatabaseConnected() {
		...
		bus.subscribeToCommand(webapp.shared.topics.ADD_ORDER_TO_ORDERS_LIST, addOrderToOrdersList);
		bus.subscribeToCommand(webapp.shared.topics.REMOVE_ORDER_FROM_ORDERS_LIST, removeOrderFromOrdersList);



		var topicsToTransmit = [webapp.shared.topics.CHAT_BROADCAST, webapp.shared.topics.ACTUALLYNOTATHOME, webapp.shared.topics.ACTUALORDER];
